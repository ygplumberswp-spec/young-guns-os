...
async function main() {
  ...
  report.api = await fetchLiveMap(token);
  ...
  for (const reg of TARGET_REGS) {
    ...
    const panelText = await page.locator('.fleet-live-map-detail, .fleet-live-map-mobile-drawer').innerText();
    markerDetails[reg] = {
      clicked: true,
  ...
  if (report.consoleErrors.some((line) => /leaflet|tile|map/i.test(line))) {
