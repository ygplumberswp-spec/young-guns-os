/**
 * FRZ-018d Xero read-only staging post-sync verification.
 ...
 *
 * Usage:
 *   node diagnostic-output/frz018d-xero-staging-post-sync-verify.mjs
 */
...
const outPath = path.resolve(repoRoot, 'diagnostic-output/175-frz018d-xero-staging-post-sync-verify.json');
...
async function signupProbe(suffix) {
  const signup = await api('/api/v1/auth/signup', {
    ...
    body: {
      ...
      firstName: 'Frz',
      lastName: 'EighteenD',
      email: `frz018d.${suffix}@staging-frz018d.test`,
...
async function runOwnerLiveChecklist(report, token) {
  ...
  if (fullSync.status === 200) {
    pass(report.results, 'checklist_bank_transactions_import', 'org verify sync ok');
    report.checklist.bankTransactionsImport = 'PASS';
  } else {
    fail(report.results, 'checklist_bank_transactions_import', JSON.stringify(fullSync.json?.error));
...
async function main() {
  const report = {
    schemaVersion: 'frz018d-staging-post-sync-verify-v1',
    sprint: 'FRZ-018d',
    startedAt: new Date().toISOString(),
    branch: 'cursor/titan-frozen-scope-completion',
    priorCheckpoint: 'fde527c',
    priorSprint: 'FRZ-018c',
    ...
    readOnlyIntent: true,
    ownerSignal: 'xero synced — Sync now clicked on staging /integrations/xero',
  ...
  if (OWNER_ACCESS_TOKEN) {
    ...
    report.ownerAction = {
      step: 1,
      action: 'Retry Sync from staging /integrations/xero (entity sync buttons) or Integrations dashboard Sync now; note any UI error',
      step2: 'Optional: re-run with OWNER_ACCESS_TOKEN for live read-only import verify',
      note: 'FRZ-018d DB found no sync evidence despite Owner signal',
    };
  }
