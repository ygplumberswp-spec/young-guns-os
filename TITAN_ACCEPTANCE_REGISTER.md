...
| FRZ-010 | §11 | Job numbering + Xero finance chain | Finance | Yes | Yes | Partial | Yes | Yes | Partial | Yes | Yes | UX-E staging | **Xero** | Live write | Partially implemented | Read import + approval gate scaffold (Sprint 186); live gated write + official # on push pending verify GO | Cursor | Sprint 186 | `TITAN_XERO_TWO_WAY_SYNC.md`, `xero-write-approval-gate.test.ts`, `JobCompletionFinancePanel.tsx` |
...
| FRZ-018 | §19 | Integrations truthful provider states | Owner | Yes | Yes | Partial | Yes | Yes | Partial | Yes | Yes | **PARTIAL** — OAuth connected; import auto-resume; two-way scaffold queued | **Multi** | OAuth | Partially implemented | OAuth + tenant isolation PASS; FRZ-018h heartbeat recovery deployed; two-way verify after GO | Owner | Sprint FRZ-018h | `TITAN_FRZ018_XERO_STAGING_REPORT.md`, `187-xero-import-recovery-verify.json` |
...
11. **Secure persistent session (complete-app)** — **DONE (local automated + staging script)** — HttpOnly cookie restore, cross-tab sync, reuse detection, step-up auth, `/settings/security`; see `TITAN_SECURE_SESSION_ARCHITECTURE.md`, `184-secure-session-staging-verify.json`  
12. **OPS-013 / offline duplicate-completion** — **DONE (local automated)** — `mobile-offline-completion.test.ts` + `job-execution-completion-idempotency.test.ts` + route contract; staging click-path still blocked (503/28P01)  
13. **FRZ-002 / role-forbidden direct URL** — **DONE (local automated)** — `role-forbidden-direct-url.test.ts` + `StaffExperienceRoute` redirect contract; staging click-path **GO** (Sprint 022)  
...
15. **FRZ-018** — Xero OAuth staging connect — **PARTIAL** (OAuth connected; SCHEDULERS_ENABLED; 49 contacts; reconnect Xero for auto-sync schedule + FRZ-018f GO)  
