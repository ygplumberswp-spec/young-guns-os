/**
 * Apply ONLY 0113_company_day_plans on staging (Day Planning).
...
const MIGRATION_TAG = '0113_company_day_plans';
...
try {
  ...
  const tableBefore = await db`
    select to_regclass('public.company_day_plans') as reg
  `;
    ...
    JSON.stringify(
      {
        ...
        companyDayPlansTable: tableAfter[0].reg,
