/**
 * Apply ONLY 0180_fb_oauth_initiator_role on staging.
...
const MIGRATION_TAG = '0180_fb_oauth_initiator_role';
const PRIOR_TAG = '0179_social_connection_foundation';
...
if (prior?.tag !== PRIOR_TAG) {
  console.error(`NO-GO: expected ${PRIOR_TAG} immediately before 0180, found ${prior?.tag ?? 'none'}`);
  process.exit(2);
}
...
try {
  ...
  const count0179 = appliedTags.filter((t) => t === PRIOR_TAG).length;
  const count0180 = appliedTags.filter((t) => t === MIGRATION_TAG).length;
  const has0180Hash = migRows.some((row) => row.hash === hash);
  ...
  const precheck = {
    ...
    schemaBefore: {
      fbOauthInitiatorRole: await columnExists('fb_oauth_states', 'initiator_role_name'),
      socialOauthInitiatorRole: await columnExists('social_oauth_states', 'initiator_role_name'),
    },
  ...
  if (count0179 !== 1) {
    console.error(`NO-GO: 0179 must be applied exactly once (found ${count0179})`);
    process.exit(3);
  }
  if (count0180 > 0 || has0180Hash) {
    console.error('NO-GO: 0180 already applied');
    process.exit(3);
  }
  ...
  const post = {
    ...
    tag: MIGRATION_TAG,
    hash,
    count0180Applied: appliedTagsAfter.filter((t) => t === MIGRATION_TAG).length,
    schemaAfter: {
      fbOauthInitiatorRole: await columnExists('fb_oauth_states', 'initiator_role_name'),
      socialOauthInitiatorRole: await columnExists('social_oauth_states', 'initiator_role_name'),
    },
  ...
  if (
    !post.schemaAfter.fbOauthInitiatorRole ||
    !post.schemaAfter.socialOauthInitiatorRole ||
    post.count0180Applied !== 1 ||
    !rowCountsUnchanged
  ) {
