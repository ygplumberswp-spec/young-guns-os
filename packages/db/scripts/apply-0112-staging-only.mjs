/**
 * Apply ONLY 0112_draft_workspace on staging (Phase 1 polish draft foundation).
...
const MIGRATION_TAG = '0112_draft_workspace';
...
try {
  ...
  const tableBefore = await db`
    select to_regclass('public.draft_workspace') as reg
  `;
    ...
    JSON.stringify(
      {
        ...
        draftWorkspaceTable: tableAfter[0].reg,
        wasPresentBefore: Boolean(tableBefore[0].reg),
      },
      null,
      2,
    ),
