...
| UX-002 | accessibility | Young Guns dark theme consistent (f8cc0c4 theme pass) | BUILT LOCALLY | YES | YES | NO | NO | NO | NO | NO | NO | NO | NO | YES | YES | NO | NO | young-guns-theme.test.ts @ f8cc0c4 | f8cc0c4 |  | main teal drift; full standard → YG-VIS | Execute deferred YG-VIS after functional sequence |
...
| Young Guns theme on all 155 pages | App-wide | f8cc0c4 covers finance/docs; full premium standard deferred to **YG-VIS** | UX-002 partial; VIS-001–010 |
...
|-------|-------|--------|--------|
| **Theme cleanup** | Superseded by deferred **YG-VIS** (full Young Guns Premium Dark Brand Visual System) | `docs/TITAN_YG_VISUAL_SYSTEM_MIGRATION_MATRIX.md` | **SUPERSEDED → YG-VIS** |
...
| **YG-VIS — Young Guns Premium Dark Brand Visual System** | One consistent premium YG identity app-wide (tokens, shell, components, docs stay print-light) | Owner direction 2026-08-05; migration matrix | **DEFERRED — LAST** — see end of this document |
...
*Generated requirement count: **166** rows (+ **YG-VIS** deferred register below — not started). Update this document when any row changes classification; do not maintain competing checklists elsewhere.*

---

## Deferred Phase: Young Guns Premium Dark Brand Visual System (YG-VIS)

> **MASTER-SEQUENCE RULE:** Do **not** start this phase while an unrelated active phase is in flight (including J-6.6A and master sequence steps 3–16).  
> Add / execute only after the approved functional sequence reaches an Owner-approved pause point.  
> Do **not** overwrite approved functionality, architecture, routes, permissions, or business logic.  
> Do **not** push, deploy, or release visual implementation without **explicit Owner approval**.

| Field | Value |
|-------|-------|
| **Phase ID** | **YG-VIS** |
| **Status** | **DEFERRED — NOT STARTED** |
| **Position** | Absolute end of backlog after functional/integration gates |
| **Planning artifact** | `docs/TITAN_YG_VISUAL_SYSTEM_MIGRATION_MATRIX.md` |
| **Supersedes** | Future-phase row **Theme cleanup** |
| **Prior partial work** | `f8cc0c4` Young Guns theme + document engine (recovery lineage) — reuse; do not rebuild |
| **Current `main` drift** | Legacy electric **teal/cyan** (`#22d3ee` / `#06b6d4`) in `packages/ui/src/tokens.css`, shell CSS, wordmark |

### Owner visual direction (approved)

- Black and deep navy foundations; electric-blue accents; crisp white text; chrome/silver detail; controlled glossy highlights
- Premium Cape Town plumbing identity; strong contrast; clean spacing; bold but uncluttered; luxury OS neatness
- Feeling: Dark · Electric blue · Chrome · Clean · Bold · Professional · Highly readable · Consistent · Modern · Powerful · Very neat
- Official slogan: **Your #2 Is Our #1 Priority.**  
  Never: “Your #2 Is Our #2 Priority.”
- Approved public details: Phone **066 234 6301** · Email **ygplumberswp@gmail.com** · Location **Cape Town, Western Cape** · Business **Young Guns Plumbing Cape Town**

### Token foundation (central only)

Suggested Owner palette — map into existing `--titan-*` aliases; prefer one canonical `--yg-*` scheme + aliases over scattered hex:

| Token | Value |
|-------|-------|
| `--yg-bg` | `#05070A` |
| `--yg-surface` | `#0A0F18` |
| `--yg-surface-raised` | `#101827` |
| `--yg-blue` | `#079CFF` |
| `--yg-blue-bright` | `#18B9FF` |
| `--yg-blue-deep` | `#0568D8` |
| `--yg-white` | `#F7FAFC` |
| `--yg-silver` | `#B8C2CF` |
| `--yg-muted` | `#7C8796` |
...
Plus controlled success green, warning amber, error red, Google-style yellow review stars.  
Reuse approved TITAN tokens already established on the recovery Young Guns pass where compatible.

### Visual balance (must remain usable)

**Use:** electric-blue focus rings; thin blue/chrome borders; controlled card highlights; strong typography; clean icons; subtle shadows; minimal glow; carefully placed water/ripple motifs; high-quality empty states where appropriate.

**Do not use:** water behind tables; heavy glow on every control; competing gradients; busy textured form backgrounds; chrome body copy; readability-hurting decoration; excessive animation; neon overload; fake 3D buttons; generic AI plumbing graphics throughout.

The app must feel inspired by branding without looking like a promotional poster on every screen.

### Typography

- Bold page titles; clear section headings; readable body; compact tables; strong monetary totals; clear status labels; consistent form labels
- Wordmark: **Y**oung / **G**uns / **P**lumbing initials in blue; remaining letters white or silver
- No decorative script inside operational UI; script slogan only on login, branded exports, marketing, controlled brand areas

### App shell scope (one consistent shell)

Owner Command Center · AURA Chat · Finance · Customers · Jobs · Dispatch · Fleet · Technicians · Inventory · Warehouse · Suppliers · Marketing · Reports · Integrations · Settings · Client Portal · Technician mobile.

Required: deep dark nav; clear active state with electric-blue indicator; brilliant white page titles; consistent card radius/borders/spacing; breadcrumbs where useful; strong responsive behaviour. Owner view premium/commanding; technician practical/fast; client portal clean/simple — do not over-complex Client or Technician screens.

### Component + data rules

Standardize primary/secondary/destructive buttons, inputs, selects, date pickers, tables, cards, modals, drawers, tabs, badges, alerts, empty/loading/skeletons, charts, tooltips, pagination, uploads, galleries, PDF previews.

- Primary: solid electric blue, white text, clear hover/focus/disabled
- Secondary: dark surface, blue/chrome border, white text
- No teal/cyan legacy that conflicts with Young Guns blue
- Status never colour-alone — labels (+ icons); accessible contrast
- Charts: genuine provider/DB data only; honest empties; restrained YG colours; dark-readable; labels + accessible summaries
- Documents/reports: **white/pearl** background, black body, YG blue headings, YGP blue initials, large totals/bank details, yellow review stars, genuine Yoco/QR/COC only — dark app theme must **not** force documents dark

### Water / chrome placement

**Appropriate:** login; Owner dashboard header; AURA command surface; empty states; document covers; report headers; marketing; loading/system-health; branded onboarding; profile/company branding.

**Avoid behind:** dense tables; form fields; invoice line editors; job cards; maps; settings; compliance forms.

Chrome = subtle highlights/borders — not main text treatment.

### Accessibility

WCAG-conscious contrast · keyboard navigation · focus-visible · text status labels · accessible form labels & modal titles · mobile touch targets · no info via glow/colour alone · reduced-motion · responsive at **1440 / 1024 / 768 / 390**.

### Implementation approach (when unlocked)

1. Audit tokens/components (matrix already drafted)
2. Identify reusable approved styling
3. Identify legacy colours / inconsistent components
4. Follow `docs/TITAN_YG_VISUAL_SYSTEM_MIGRATION_MATRIX.md`
5. Apply design system centrally
6. Update screens in controlled batches
7. Typecheck, build, and full tests after **every** batch
8. Visual checks at all required widths
9. Update this checklist honestly
10. **Stop for Owner visual review before production**

Non-goals: do not rebuild the app; do not change business logic; do not introduce fake data.

### YG-VIS deferred register

| ID | Deliverable | Status | Next action when unlocked |
|----|-------------|--------|---------------------------|
| VIS-001 | Central `--yg-*` / `--titan-*` token alignment to Owner electric-blue palette; purge teal/cyan | NOT STARTED | Batch B0 — tokens + `@titan/ui` |
| VIS-002 | Restore/align `young-guns-theme` shared source + slogan constant (`#1` priority wording) | NOT STARTED | Batch B0 — shared package |
| VIS-003 | App shell / nav / wordmark (YGP blue initials) + auth login brand surface | NOT STARTED | Batch B1 |
| VIS-004 | Owner Command Center + AURA command surface premium polish (controlled accents) | NOT STARTED | Batch B2 |
| VIS-005 | Operational modules shell consistency (Finance UI chrome, CRM, Jobs, Dispatch, Fleet, Inventory, etc.) | NOT STARTED | Batches B3–B6 — **UI only** |
| VIS-006 | Client Portal + Technician mobile — clean/simple/practical variants of same system | NOT STARTED | Batch B7 |
| VIS-007 | Component standard (buttons, inputs, tables, modals, badges, empties, charts…) | NOT STARTED | Parallel with B0–B8 |
| VIS-008 | Document/report shell remains print-light pearl/white; genuine Yoco/QR/COC; yellow review stars | NOT STARTED | Batch B3 docs — preserve approved direction |
| VIS-009 | Hardcoded colour / dual-system purge; no competing style systems | NOT STARTED | Batch B9 gate |
| VIS-010 | A11y + responsive visual QA (1440/1024/768/390) + Owner visual review gate | NOT STARTED | Batch B10 — **Owner stop** |

### YG-VIS completion gate (do not mark complete until all true)

- [ ] Desktop checked
- [ ] Tablet checked
- [ ] Mobile checked
- [ ] Dark theme consistent
- [ ] No legacy teal/cyan remains
- [ ] No unreadable contrast
- [ ] No clipped components
- [ ] No inconsistent button systems
- [ ] No duplicate style systems
- [ ] No unrelated functionality changed
- [ ] Owner visual review passed
- [ ] Explicit Owner approval before any production release of this phase

### Final target

TITAN must feel like **one** premium Young Guns operating system: dark, electric blue, chrome, clean, bold, professional, highly readable, consistent, modern, powerful, and very neat.
