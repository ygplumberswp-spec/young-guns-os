...
const DEFAULT_CHECKLIST = [
  {
    itemKey: 'integration_validation',
    itemName: 'Integration validation complete',
    category: 'validation',
    isRequired: true,
  },
  {
    itemKey: 'workflow_validation',
    itemName: 'Workflow validation complete',
    category: 'validation',
    isRequired: true,
  },
  {
    itemKey: 'security_verification',
    itemName: 'Security verification complete',
    category: 'security',
    isRequired: true,
  },
  {
    itemKey: 'configuration_review',
    itemName: 'Configuration review complete',
    category: 'configuration',
    isRequired: true,
  },
  {
    itemKey: 'performance_review',
    itemName: 'Performance optimization review',
    category: 'performance',
    isRequired: false,
  },
  {
    itemKey: 'backup_verified',
    itemName: 'Backup verification',
    category: 'continuity',
    isRequired: true,
  },
  {
    itemKey: 'launch_center_approved',
    itemName: 'Launch Center go-live approved',
    category: 'release',
    isRequired: true,
  },
  {
    itemKey: 'manual_smoke_test',
    itemName: 'Manual smoke test in staging',
    category: 'manual',
    isRequired: true,
  },
  {
    itemKey: 'release_notes',
    itemName: 'Release notes prepared',
    category: 'documentation',
    isRequired: false,
  },
  {
    itemKey: 'owner_sign_off',
    itemName: 'Owner sign-off for production release',
    category: 'release',
    isRequired: true,
  },
];
...
export class EnterpriseReleaseCenterReleaseCandidateService {
  ...
  async ensureChecklist(companyId: string): Promise<void> {
    for (const item of DEFAULT_CHECKLIST) {
      const existing = await this.deps.db.query.rcReleaseChecklistItems.findFirst({
        where: and(
          eq(rcReleaseChecklistItems.companyId, companyId),
          eq(rcReleaseChecklistItems.itemKey, item.itemKey),
        ),
      });
      if (!existing) {
  ...
  async runSecurityVerification(scope: StaffScope): Promise<RcSecurityVerificationRunSummary> {
    ...
    const findings = [
      {
        key: 'risk_alerts',
        count: dashboard.riskAlertCount,
        severity: dashboard.riskAlertCount > 0 ? 'high' : 'info',
      },
      {
        key: 'failed_logins_24h',
        count: dashboard.failedLoginCount24h,
        severity: dashboard.failedLoginCount24h > 10 ? 'warning' : 'info',
      },
      {
        key: 'mfa_adoption',
        value: dashboard.mfaAdoptionPercent,
        severity:
          dashboard.mfaAdoptionPercent != null && dashboard.mfaAdoptionPercent < 50
            ? 'warning'
            : 'info',
      },
      {
        key: 'audit_events_24h',
        count: dashboard.auditEventCount24h,
        severity: dashboard.auditEventCount24h === 0 ? 'warning' : 'info',
      },
      {
        key: 'encryption',
        status: dashboard.encryption.integrationCredentialsEncrypted ? 'configured' : 'missing',
        severity: dashboard.encryption.integrationCredentialsEncrypted ? 'info' : 'critical',
      },
    ];
    ...
    const status: RcValidationStatus =
    ...
    const [created] = await this.deps.db
      ...
      .values({
        ...
        report: {
          ...
          note: 'Findings only — no security changes applied.',
        },
  ...
  async runConfigurationReview(scope: StaffScope): Promise<RcConfigurationReviewSummary> {
    ...
    if (!this.deps.encryptionKey)
      findings.push({
        ...
        severity: 'warning',
        message: 'Integration encryption key not configured.',
      });
    ...
    const [connections, adapters] = await Promise.all([
      ...
      this.deps.db.query.ucProviderAdapters.findMany({
        where: eq(ucProviderAdapters.companyId, scope.companyId),
      }),
    ]);
    ...
    if (connections.filter((c) => c.status === 'error').length > 0) {
      findings.push({
        ...
        severity: 'high',
        message: `${connections.filter((c) => c.status === 'error').length} integration(s) in error.`,
      });
    }
    if (adapters.filter((a) => a.status === 'active').length === 0) {
      findings.push({
        ...
        severity: 'warning',
        message: 'No active UC provider adapters configured.',
      });
    }
  ...
  async generateReleaseReport(scope: StaffScope): Promise<RcReleaseCandidateReportSummary> {
    ...
    const warningCount =
      integrationRun.warningCount +
      workflowRun.warningCount +
      securityRun.warningCount +
      configReview.warningCount;
    ...
    const criticalBlockers =
      integrationRun.failedCount +
      workflowRun.failedCount +
      securityRun.criticalCount +
      configReview.missingConfigCount;
    ...
    const report = {
      integrationRun: {
        ...
        failedCount: integrationRun.failedCount,
      },
      workflowRun: {
        id: workflowRun.id,
        status: workflowRun.status,
        failedCount: workflowRun.failedCount,
      },
      securityRun: {
        id: securityRun.id,
        status: securityRun.status,
        criticalCount: securityRun.criticalCount,
      },
    ...
    await this.updateChecklistFromReport(
      scope.companyId,
      integrationRun.status,
      workflowRun.status,
      securityRun.status,
      configReview.missingConfigCount === 0,
    );
  ...
  async getLatestSecurityVerification(
    companyId: string,
  ): Promise<RcSecurityVerificationRunSummary | null> {
  ...
  async getLatestConfigurationReview(
    companyId: string,
  ): Promise<RcConfigurationReviewSummary | null> {
    ...
    for (const [key, ok] of updates) {
      ...
      await this.deps.db
        ...
        .where(
          and(
            eq(rcReleaseChecklistItems.companyId, companyId),
            eq(rcReleaseChecklistItems.itemKey, key),
          ),
        );
    }
  }
...
function toChecklistSummary(
  row: typeof rcReleaseChecklistItems.$inferSelect,
): RcReleaseChecklistItemSummary {
  return {
    id: row.id,
    itemKey: row.itemKey,
...
function toSecuritySummary(
  row: typeof rcSecurityVerificationRuns.$inferSelect,
): RcSecurityVerificationRunSummary {
  return {
    id: row.id,
    runKey: row.runKey,
...
function toConfigSummary(
  row: typeof rcConfigurationReviews.$inferSelect,
): RcConfigurationReviewSummary {
  return {
    id: row.id,
...
function toReportSummary(
  row: typeof rcReleaseCandidateReports.$inferSelect,
): RcReleaseCandidateReportSummary {
  return {
    id: row.id,
