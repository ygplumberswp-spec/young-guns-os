...
const KEYWORD_DOMAINS: Array<{ pattern: RegExp; domains: AuraContextDomain[] }> = [
  ...
  { pattern: /\b(job|work order|dispatch|schedule|scheduling|appointment)\b/i, domains: ['jobs', 'scheduling', 'dispatchIntelligence'] },
  { pattern: /\b(invoice|payment|finance|accounting|xero|revenue|expense|profit)\b/i, domains: ['finance', 'financeIntelligence', 'integrations'] },
  { pattern: /\b(inventory|stock|parts|warehouse)\b/i, domains: ['inventory', 'procurement'] },
  { pattern: /\b(fleet|vehicle|driver|gps|tracking)\b/i, domains: ['fleet', 'fleetIntelligence', 'integrations'] },
  { pattern: /\b(email|sms|message|communication|whatsapp|call|voice)\b/i, domains: ['communications', 'whatsapp', 'voice', 'communicationsIntelligence'] },
  { pattern: /\b(document|file|contract|attachment)\b/i, domains: ['documents'] },
  { pattern: /\b(automation|workflow|trigger)\b/i, domains: ['automation', 'enterpriseAutomationStudio'] },
  ...
  { pattern: /\b(recruit|hiring|candidate|workforce|technician|employee)\b/i, domains: ['recruiting', 'workforce', 'mobileWorkforce'] },
  { pattern: /\b(integration|api|webhook|sync)\b/i, domains: ['integrations', 'integrationPlatform'] },
  { pattern: /\b(analytic|report|dashboard|metric|kpi|bi)\b/i, domains: ['analytics', 'businessIntelligence', 'enterpriseAnalytics'] },
  ...
  { pattern: /\b(executive|mission control|digital twin|evolution)\b/i, domains: ['executive', 'enterpriseMissionControl', 'enterpriseDigitalTwin', 'enterpriseEvolution'] },
  ...
  { pattern: /\b(saas|platform|developer)\b/i, domains: ['enterpriseSaasPlatform', 'enterpriseDeveloperPlatform'] },
];
...
function addPageContextDomains(pageContext: AuraPageContext | undefined, domains: Set<AuraContextDomain>) {
  if (!pageContext) {
    return;
  }
...
export function listSkippedAuraContextDomains(selected: Set<AuraContextDomain>): AuraContextDomain[] {
