...
const CATEGORY_REQUIREMENTS: Record<
  AiRoutingCategory,
  { requiredCapabilities: AiCapabilityFlag[]; preferLowLatency: boolean; preferLongContext: boolean }
> = {
  reasoning: { requiredCapabilities: ['reasoning'], preferLowLatency: false, preferLongContext: false },
  coding: { requiredCapabilities: ['function_calling'], preferLowLatency: false, preferLongContext: false },
  business_analysis: { requiredCapabilities: [], preferLowLatency: false, preferLongContext: false },
  finance: { requiredCapabilities: ['structured_output'], preferLowLatency: false, preferLongContext: false },
  legal: { requiredCapabilities: ['max_context'], preferLowLatency: false, preferLongContext: true },
  marketing: { requiredCapabilities: [], preferLowLatency: true, preferLongContext: false },
  image_understanding: { requiredCapabilities: ['vision', 'multimodal'], preferLowLatency: false, preferLongContext: false },
  document_analysis: { requiredCapabilities: ['max_context'], preferLowLatency: false, preferLongContext: true },
  long_context_analysis: { requiredCapabilities: ['max_context'], preferLowLatency: false, preferLongContext: true },
...
export function estimateModelCostCents(
  ...
  const inputPer1k = Number(pricingMetadata.inputPer1kTokensCents ?? pricingMetadata.inputCostPer1k ?? 0);
  const outputPer1k = Number(pricingMetadata.outputPer1kTokensCents ?? pricingMetadata.outputCostPer1k ?? 0);
