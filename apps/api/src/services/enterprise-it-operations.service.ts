...
import {
  itoAnalyticsSnapshots,
  itoAuditLogs,
  itoAiProviderHealth,
  itoApiReliabilitySnapshots,
  itoBackupVerifications,
  itoBugDetections,
  itoBuildRecords,
  itoChangeRequests,
  itoDatabaseHealthSnapshots,
  itoDependencyRecords,
  itoDeployments,
  itoHealthMonitors,
  itoHealthSnapshots,
  itoIncidents,
  itoIntegrationHealth,
  itoItActionDrafts,
  itoItAlerts,
  itoPerformanceSnapshots,
  itoPlatformConfig,
  itoRepairAttempts,
  itoRootCauseAnalyses,
  itoSelfHealingActions,
  itoTechnicalDebtRecords,
  itoTestRuns,
...
export class EnterpriseItOperationsError extends Error {
  constructor(
    ...
    super(message);
    this.name = 'EnterpriseItOperationsError';
  }
}

type StaffScope = { companyId: string; userId: string };
type ExecuteSafeRepairRequest = { repairKey: string; input?: Record<string, unknown> };
type ExecuteSafeRepairResult = {
  repairAttemptId: string;
  selfHealingActionId: string;
  verified: boolean;
  workflowStatus: string;
  output: Record<string, unknown>;
};
type CreateItoHealthSnapshotRequest = {
  monitorId?: string;
  snapshotKey: string;
  healthStatus?: string;
...
export class EnterpriseItOperationsService {
  ...
  async getDashboard(companyId: string): Promise<EnterpriseItOperationsDashboard> {
    ...
    const openBugCount = bugs.filter(
      (b) => !['executed', 'cancelled'].includes(b.workflowStatus),
    ).length;
    const pendingChangeRequestCount = changes.filter((c) =>
      ['draft', 'review', 'pending_approval', 'approved'].includes(c.workflowStatus),
    ).length;
    const failedDeploymentCount = deployments.filter((d) => d.deploymentStatus === 'failed').length;
    const technicalDebtCount = debt.filter(
      (d) => !['executed', 'cancelled'].includes(d.workflowStatus),
    ).length;
    return {
  ...
  async getOperationsMonitoring(companyId: string): Promise<ItoOperationsMonitoringSummary> {
    ...
    const degradedMonitorCount =
      monitors.filter((m) => m.healthStatus !== 'healthy').length +
      degradedModules +
      unhealthyModules;
    const openIncidentCount = incidents.filter(
      (i) => !['resolved', 'closed'].includes(i.status),
    ...
    const integrationIssueCount =
      integrationMonitoring.errorServiceCount + integrationDashboard.recentConflicts.length;
    const criticalSignals =
      missionControlDashboard.criticalAlertCount +
      alerts.filter((a) => a.severity === 'critical').length +
      securityDashboard.riskAlertCount;
    ...
    if (aiResilience.recentFailoverCount > 0)
      alertsList.push(`${aiResilience.recentFailoverCount} recent AI failover(s)`);
    if (criticalSignals > 0) alertsList.push(`${criticalSignals} critical signal(s)`);
    const overallHealthStatus =
      unhealthyModules > 0 || openIncidentCount > 0
        ? 'unhealthy'
        : degradedModules > 0 || criticalSignals > 0
          ? 'degraded'
          : 'healthy';
    return {
      openIncidentCount,
  ...
  async updatePlatformConfig(
    scope: StaffScope,
    input: UpdateItoPlatformConfigRequest,
  ): Promise<ItoPlatformConfigSummary> {
    const existing = await this.ensurePlatformConfig(scope.companyId);
    const [updated] = await this.deps.db
      .update(itoPlatformConfig)
      .set({
      ...
      .where(eq(itoPlatformConfig.companyId, scope.companyId))
      .returning();
    await this.recordAudit(scope, 'platform_config_updated');
    return toPlatformConfigSummary(updated!);
  }

  async recordSelfHealingAction(
    scope: StaffScope,
    input: CreateItoSelfHealingActionRequest,
  ): Promise<ItoSelfHealingActionSummary> {
    const config = await this.getPlatformConfig(scope.companyId);
    const allowlist =
      (config.healingPolicies.allowlist as
        Array<{ actionType: string; riskLevel: string }> | undefined) ?? [];
    const risk = input.riskLevel ?? 'low';
    const allowed = allowlist.some((entry) => entry.actionType === input.actionType);
    if (!allowed && risk !== 'low')
      throw new EnterpriseItOperationsError(
        'VALIDATION_ERROR',
        'Self-healing action is not in configured allowlist',
      );
    return this.createSelfHealingAction(scope, {
      ...input,
      riskLevel: risk,
      triggeredBy: input.triggeredBy ?? scope.userId,
      config: input.config ?? {},
    });
  }

  async listSelfHealingActions(companyId: string): Promise<ItoSelfHealingActionSummary[]> {
    const rows = await this.deps.db.query.itoSelfHealingActions.findMany({
      where: eq(itoSelfHealingActions.companyId, companyId),
      orderBy: [desc(itoSelfHealingActions.createdAt)],
      limit: 100,
    });
  ...
  async syncBugDetections(scope: StaffScope): Promise<ItoBugDetectionSummary[]> {
    ...
    const [failedTestRuns, failedBuilds, failedDeployments, recentLogs, failedWorkflows, existing] =
      await Promise.all([
        this.deps.db.query.itoTestRuns.findMany({
          where: and(eq(itoTestRuns.companyId, companyId)),
          orderBy: [desc(itoTestRuns.completedAt)],
          limit: 50,
        }),
        this.deps.db.query.itoBuildRecords.findMany({
          where: eq(itoBuildRecords.companyId, companyId),
          orderBy: [desc(itoBuildRecords.completedAt)],
          limit: 50,
        }),
        this.deps.db.query.itoDeployments.findMany({
          where: and(
            eq(itoDeployments.companyId, companyId),
            eq(itoDeployments.deploymentStatus, 'failed'),
          ),
          orderBy: [desc(itoDeployments.completedAt)],
          limit: 50,
        }),
        this.deps.db.query.opsOperationalLogEntries.findMany({
          where: and(
            eq(opsOperationalLogEntries.companyId, companyId),
            inArray(opsOperationalLogEntries.severity, ['error', 'critical']),
          ),
    ...
    for (const run of failedTestRuns.filter(
      (r) => r.failedCount > 0 || r.workflowStatus === 'cancelled',
    )) {
      const fingerprint = `test_run:${run.id}`;
      if (!existing.some((row) => row.sourceModule === fingerprint)) {
        await this.createBugDetection(scope, {
          detectionSource: 'test_run',
          severity: 'critical',
          title: `Failed test run ${run.runKey}`,
          description: `${run.failedCount} failed test(s) in ${run.testSuite}`,
          sourceModule: fingerprint,
    ...
    for (const build of failedBuilds.filter((b) => b.workflowStatus === 'cancelled')) {
      ...
      if (!existing.some((row) => row.sourceModule === fingerprint)) {
        await this.createBugDetection(scope, {
          detectionSource: 'build',
          severity: 'critical',
          title: `Failed build ${build.buildKey}`,
          description: build.branch ? `Build failed on ${build.branch}` : 'Build failed',
          sourceModule: fingerprint,
    ...
    for (const log of recentLogs) {
      ...
      if (!existing.some((row) => row.sourceModule === fingerprint)) {
        await this.createBugDetection(scope, {
          detectionSource: 'operational_log',
          severity: log.severity === 'critical' ? 'critical' : 'warning',
          title: log.message.slice(0, 200),
          description: log.moduleKey,
          sourceModule: fingerprint,
    ...
    for (const workflow of failedWorkflows) {
      ...
      if (!existing.some((row) => row.sourceModule === fingerprint)) {
        await this.createBugDetection(scope, {
          detectionSource: 'workflow_run',
          severity: 'warning',
          title: `Failed workflow ${workflow.triggerEvent}`,
          sourceModule: fingerprint,
  ...
  async createIncident(
    scope: StaffScope,
    input: CreateItoIncidentRequest,
  ...
  async listIncidents(companyId: string): Promise<ItoIncidentSummary[]> {
    const rows = await this.deps.db.query.itoIncidents.findMany({
  ...
  async getIncident(companyId: string, id: string): Promise<ItoIncidentSummary | null> {
    const row = await this.deps.db.query.itoIncidents.findFirst({
      where: and(eq(itoIncidents.companyId, companyId), eq(itoIncidents.id, id)),
    });
  ...
  async updateIncident(
    scope: StaffScope,
    id: string,
    input: UpdateItoIncidentRequest,
  ): Promise<ItoIncidentSummary> {
    await this.ensureIncident(scope.companyId, id);
    const [updated] = await this.deps.db
      .update(itoIncidents)
      .set({
        ...(input.incidentNumber !== undefined
          ? { incidentNumber: input.incidentNumber ?? null }
          : {}),
        ...
        ...(input.severity !== undefined
          ? { severity: input.severity as typeof itoIncidents.$inferInsert.severity }
          : {}),
        ...(input.status !== undefined
          ? { status: input.status as typeof itoIncidents.$inferInsert.status }
          : {}),
        ...
        ...(input.mitigatedAt !== undefined
          ? { mitigatedAt: parseOptionalDate(input.mitigatedAt) }
          : {}),
        ...(input.resolvedAt !== undefined
          ? { resolvedAt: parseOptionalDate(input.resolvedAt) }
          : {}),
        ...(input.config !== undefined ? { config: input.config } : {}),
        updatedAt: new Date(),
      })
      .where(and(eq(itoIncidents.companyId, scope.companyId), eq(itoIncidents.id, id)))
      .returning();
  ...
  ): Promise<ItoItAlertSummary[]> {
    const rows = await this.deps.db.query.itoItAlerts.findMany({
      where: eq(itoItAlerts.companyId, companyId),
  ...
  async syncItAlerts(scope: StaffScope): Promise<ItoItAlertSummary[]> {
    ...
    const defs = [
      [
        ...
        'critical',
        'Open incidents',
        `${monitoring.openIncidentCount} open incident(s)`,
        monitoring.openIncidentCount > 0,
      ],
      [
        'degraded_monitors',
        'warning',
        'Degraded monitors',
        `${monitoring.degradedMonitorCount} degraded monitor(s)`,
        monitoring.degradedMonitorCount > 0,
      ],
      [
        'open_bugs',
        'warning',
        'Open bug detections',
        `${monitoring.openBugCount} open bug(s)`,
        monitoring.openBugCount > 0,
      ],
      [
        'failed_deployments',
        'critical',
        'Failed deployments',
        `${monitoring.failedDeploymentCount} failed deployment(s)`,
        monitoring.failedDeploymentCount > 0,
      ],
      [
        'pending_changes',
        'warning',
        'Pending change requests',
        `${monitoring.pendingChangeRequestCount} pending change(s)`,
        monitoring.pendingChangeRequestCount > 0,
      ],
    ] as const;
    for (const [alertType, severity, title, description, active] of defs) {
      ...
      if (active && !existing) {
        await this.deps.db.insert(itoItAlerts).values({
          companyId: scope.companyId,
          alertType,
          severity,
          status: 'open',
        ...
        await this.deps.db
          ...
          .where(eq(itoItAlerts.id, existing.id));
      }
    }
  ...
  async captureAnalytics(scope: StaffScope): Promise<ItoAnalyticsSummary> {
    const [dashboard, monitoring] = await Promise.all([
      ...
      this.getOperationsMonitoring(scope.companyId),
    ]);
    const [created] = await this.deps.db
      .insert(itoAnalyticsSnapshots)
      .values({
        companyId: scope.companyId,
        ...
        overallHealthStatus:
          monitoring.overallHealthStatus as typeof itoAnalyticsSnapshots.$inferInsert.overallHealthStatus,
      })
      .returning();
  ...
  async getLatestAnalytics(companyId: string): Promise<ItoAnalyticsSummary | null> {
    const row = await this.deps.db.query.itoAnalyticsSnapshots.findFirst({
      where: eq(itoAnalyticsSnapshots.companyId, companyId),
      orderBy: [desc(itoAnalyticsSnapshots.capturedAt)],
    });
  ...
  async executeSafeRepair(
    scope: StaffScope,
    input: ExecuteSafeRepairRequest,
  ): Promise<ExecuteSafeRepairResult> {
    const config = await this.getPlatformConfig(scope.companyId);
    const allowlist =
      (config.healingPolicies.allowlist as
        Array<{ repairKey: string; riskLevel: string }> | undefined) ?? [];
    const allowed = allowlist.find((entry) => entry.repairKey === input.repairKey);
    if (!allowed || allowed.riskLevel !== 'low')
      throw new EnterpriseItOperationsError(
        'VALIDATION_ERROR',
        'Only configured low-risk repairs are allowed',
      );
    ...
    if (input.repairKey === 'retry_integration_sync') {
      ...
      if (!connectorId)
        throw new EnterpriseItOperationsError('VALIDATION_ERROR', 'connectorId is required');
      const result = await this.deps.integrationPlatformService.retryConnectorSync(
        scope,
        connectorId,
      );
      output = { syncJobId: result.syncJobId };
      verified = result.syncJobId != null;
    } else if (input.repairKey === 'sync_mission_control_alerts') {
      await this.deps.enterpriseProductionReadinessService.syncMissionControlAlerts(
        scope.companyId,
      );
      output = { synced: true };
      verified = true;
    } else if (input.repairKey === 'capture_health_snapshots') {
      const snapshots = await this.deps.enterpriseProductionReadinessService.captureHealthSnapshots(
        scope.companyId,
      );
      output = { snapshotCount: snapshots.length };
      verified = snapshots.length > 0;
    } else throw new EnterpriseItOperationsError('VALIDATION_ERROR', 'Unsupported repair key');
    const healing = await this.recordSelfHealingAction(scope, {
      actionType: input.repairKey,
      riskLevel: 'low',
      triggeredBy: scope.userId,
      config: { input: input.input ?? {}, output },
    });
    const repairAttempt = await this.createRepairAttempt(scope, {
      repairType: input.repairKey,
      riskLevel: 'low',
      notes: verified ? 'Verified safe repair' : 'Repair executed',
      config: { input: input.input ?? {}, output },
    });
    if (verified) {
      await this.updateRepairAttempt(scope, repairAttempt.id, {
        success: true,
    ...
    return {
      ...
      selfHealingActionId: healing.id,
      verified,
      workflowStatus: repairAttempt.workflowStatus,
      output,
    };
  }
    ...
    if (productionDashboard.performance) {
      ...
      await this.deps.db.insert(itoDatabaseHealthSnapshots).values({
        ...
        connectionPoolUsagePercent:
          productionDashboard.performance.dbPoolUsagePercent?.toString() ?? null,
        slowQueryCount: 0,
        metrics: {},
      });
  ...
  private async ensurePlatformConfig(companyId: string) {
    const existing = await this.deps.db.query.itoPlatformConfig.findFirst({
      where: eq(itoPlatformConfig.companyId, companyId),
    });
    if (existing) return existing;
    const [created] = await this.deps.db
      .insert(itoPlatformConfig)
      .values({ companyId })
      .returning();
    return created!;
  }
  ...
  private async recordAudit(
    ...
    await this.deps.db.insert(itoAuditLogs).values({
      companyId: scope.companyId,
      userId: scope.userId,
      actionType,
  ...
  async createHealthMonitor(
    scope: StaffScope,
    input: CreateItoHealthMonitorRequest,
  ): Promise<ItoHealthMonitorSummary> {
    const [created] = await this.deps.db
      .insert(itoHealthMonitors)
      .values({ companyId: scope.companyId, ...mapCreateHealthMonitorInput(input) })
      .returning();
  ...
  async listHealthMonitors(companyId: string): Promise<ItoHealthMonitorSummary[]> {
    const rows = await this.deps.db.query.itoHealthMonitors.findMany({
      where: eq(itoHealthMonitors.companyId, companyId),
      orderBy: [desc(itoHealthMonitors.createdAt)],
      limit: 100,
    });
  ...
  async getHealthMonitor(companyId: string, id: string): Promise<ItoHealthMonitorSummary | null> {
    const row = await this.deps.db.query.itoHealthMonitors.findFirst({
      where: and(eq(itoHealthMonitors.companyId, companyId), eq(itoHealthMonitors.id, id)),
    });
    return row ? toHealthMonitorSummary(row) : null;
  }
  async updateHealthMonitor(
    scope: StaffScope,
    id: string,
    input: UpdateItoHealthMonitorRequest,
  ): Promise<ItoHealthMonitorSummary> {
    await this.ensureHealthMonitor(scope.companyId, id);
    const [updated] = await this.deps.db
      .update(itoHealthMonitors)
      .set({ ...mapUpdateHealthMonitorInput(input), updatedAt: new Date() })
      .where(and(eq(itoHealthMonitors.companyId, scope.companyId), eq(itoHealthMonitors.id, id)))
      .returning();
  ...
  async createHealthSnapshot(
    scope: StaffScope,
    input: CreateItoHealthSnapshotRequest,
  ): Promise<ItoHealthSnapshotSummary> {
    const [created] = await this.deps.db
      .insert(itoHealthSnapshots)
      .values({ companyId: scope.companyId, ...mapCreateHealthSnapshotInput(input) })
      .returning();
  ...
  async listHealthSnapshots(companyId: string): Promise<ItoHealthSnapshotSummary[]> {
    const rows = await this.deps.db.query.itoHealthSnapshots.findMany({
      where: eq(itoHealthSnapshots.companyId, companyId),
      orderBy: [desc(itoHealthSnapshots.capturedAt)],
      limit: 100,
    });
  ...
  async getHealthSnapshot(companyId: string, id: string): Promise<ItoHealthSnapshotSummary | null> {
    const row = await this.deps.db.query.itoHealthSnapshots.findFirst({
      where: and(eq(itoHealthSnapshots.companyId, companyId), eq(itoHealthSnapshots.id, id)),
    });
    return row ? toHealthSnapshotSummary(row) : null;
  }
  async createSelfHealingAction(
    scope: StaffScope,
    input: CreateItoSelfHealingActionRequest,
  ): Promise<ItoSelfHealingActionSummary> {
    const [created] = await this.deps.db
      .insert(itoSelfHealingActions)
      .values({ companyId: scope.companyId, ...mapCreateSelfHealingActionInput(input) })
      .returning();
    await this.recordAudit(
      scope,
      'self_healing_actions_created',
      'ito_self_healing_actions',
      created!.id,
    );
    return toSelfHealingActionSummary(created!);
  }
  async getSelfHealingAction(
    companyId: string,
    id: string,
  ): Promise<ItoSelfHealingActionSummary | null> {
    const row = await this.deps.db.query.itoSelfHealingActions.findFirst({
      where: and(eq(itoSelfHealingActions.companyId, companyId), eq(itoSelfHealingActions.id, id)),
    });
    return row ? toSelfHealingActionSummary(row) : null;
  }
  async updateSelfHealingAction(
    scope: StaffScope,
    id: string,
    input: UpdateItoSelfHealingActionRequest,
  ): Promise<ItoSelfHealingActionSummary> {
    await this.ensureSelfHealingAction(scope.companyId, id);
    const [updated] = await this.deps.db
      .update(itoSelfHealingActions)
      .set({ ...mapUpdateSelfHealingActionInput(input), updatedAt: new Date() })
      .where(
        and(eq(itoSelfHealingActions.companyId, scope.companyId), eq(itoSelfHealingActions.id, id)),
      )
      .returning();
  ...
  async createBugDetection(
    scope: StaffScope,
    input: CreateItoBugDetectionRequest,
  ): Promise<ItoBugDetectionSummary> {
    const [created] = await this.deps.db
      .insert(itoBugDetections)
      .values({ companyId: scope.companyId, ...mapCreateBugDetectionInput(input) })
      .returning();
  ...
  async listBugDetections(companyId: string): Promise<ItoBugDetectionSummary[]> {
    const rows = await this.deps.db.query.itoBugDetections.findMany({
      where: eq(itoBugDetections.companyId, companyId),
      orderBy: [desc(itoBugDetections.createdAt)],
      limit: 100,
    });
  ...
  async getBugDetection(companyId: string, id: string): Promise<ItoBugDetectionSummary | null> {
    const row = await this.deps.db.query.itoBugDetections.findFirst({
      where: and(eq(itoBugDetections.companyId, companyId), eq(itoBugDetections.id, id)),
    });
    return row ? toBugDetectionSummary(row) : null;
  }
  async updateBugDetection(
    scope: StaffScope,
    id: string,
    input: UpdateItoBugDetectionRequest,
  ): Promise<ItoBugDetectionSummary> {
    await this.ensureBugDetection(scope.companyId, id);
    const [updated] = await this.deps.db
      .update(itoBugDetections)
      .set({ ...mapUpdateBugDetectionInput(input), updatedAt: new Date() })
      .where(and(eq(itoBugDetections.companyId, scope.companyId), eq(itoBugDetections.id, id)))
      .returning();
  ...
  async createRootCauseAnalysis(
    scope: StaffScope,
    input: CreateItoRootCauseAnalysisRequest,
  ): Promise<ItoRootCauseAnalysisSummary> {
    const [created] = await this.deps.db
      .insert(itoRootCauseAnalyses)
      .values({ companyId: scope.companyId, ...mapCreateRootCauseAnalysisInput(input, scope) })
      .returning();
    await this.recordAudit(
      scope,
      'root_cause_analyses_created',
      'ito_root_cause_analyses',
      created!.id,
    );
  ...
  async listRootCauseAnalyses(companyId: string): Promise<ItoRootCauseAnalysisSummary[]> {
    const rows = await this.deps.db.query.itoRootCauseAnalyses.findMany({
      where: eq(itoRootCauseAnalyses.companyId, companyId),
      orderBy: [desc(itoRootCauseAnalyses.createdAt)],
      limit: 100,
    });
  ...
  async getRootCauseAnalysis(
    companyId: string,
    id: string,
  ): Promise<ItoRootCauseAnalysisSummary | null> {
    const row = await this.deps.db.query.itoRootCauseAnalyses.findFirst({
      where: and(eq(itoRootCauseAnalyses.companyId, companyId), eq(itoRootCauseAnalyses.id, id)),
    });
  ...
  async updateRootCauseAnalysis(
    scope: StaffScope,
    id: string,
    input: UpdateItoRootCauseAnalysisRequest,
  ): Promise<ItoRootCauseAnalysisSummary> {
    await this.ensureRootCauseAnalysis(scope.companyId, id);
    const [updated] = await this.deps.db
      .update(itoRootCauseAnalyses)
      .set({ ...mapUpdateRootCauseAnalysisInput(input), updatedAt: new Date() })
      .where(
        and(eq(itoRootCauseAnalyses.companyId, scope.companyId), eq(itoRootCauseAnalyses.id, id)),
      )
      .returning();
  ...
  async createRepairAttempt(
    scope: StaffScope,
    input: CreateItoRepairAttemptRequest,
  ): Promise<ItoRepairAttemptSummary> {
    const [created] = await this.deps.db
      .insert(itoRepairAttempts)
      .values({ companyId: scope.companyId, ...mapCreateRepairAttemptInput(input, scope) })
      .returning();
  ...
  async listRepairAttempts(companyId: string): Promise<ItoRepairAttemptSummary[]> {
    const rows = await this.deps.db.query.itoRepairAttempts.findMany({
      where: eq(itoRepairAttempts.companyId, companyId),
      orderBy: [desc(itoRepairAttempts.createdAt)],
      limit: 100,
    });
  ...
  async getRepairAttempt(companyId: string, id: string): Promise<ItoRepairAttemptSummary | null> {
    const row = await this.deps.db.query.itoRepairAttempts.findFirst({
      where: and(eq(itoRepairAttempts.companyId, companyId), eq(itoRepairAttempts.id, id)),
    });
  ...
  async updateRepairAttempt(
    scope: StaffScope,
    id: string,
    input: UpdateItoRepairAttemptRequest,
  ): Promise<ItoRepairAttemptSummary> {
    await this.ensureRepairAttempt(scope.companyId, id);
    const [updated] = await this.deps.db
      .update(itoRepairAttempts)
      .set({ ...mapUpdateRepairAttemptInput(input), updatedAt: new Date() })
      .where(and(eq(itoRepairAttempts.companyId, scope.companyId), eq(itoRepairAttempts.id, id)))
      .returning();
  ...
  async createBuildRecord(
    scope: StaffScope,
    input: CreateItoBuildRecordRequest,
  ): Promise<ItoBuildRecordSummary> {
    const [created] = await this.deps.db
      .insert(itoBuildRecords)
      .values({ companyId: scope.companyId, ...mapCreateBuildRecordInput(input) })
      .returning();
  ...
  async listBuildRecords(companyId: string): Promise<ItoBuildRecordSummary[]> {
    const rows = await this.deps.db.query.itoBuildRecords.findMany({
      where: eq(itoBuildRecords.companyId, companyId),
      orderBy: [desc(itoBuildRecords.createdAt)],
      limit: 100,
    });
    return rows.map(toBuildRecordSummary);
  }
  async getBuildRecord(companyId: string, id: string): Promise<ItoBuildRecordSummary | null> {
    const row = await this.deps.db.query.itoBuildRecords.findFirst({
      where: and(eq(itoBuildRecords.companyId, companyId), eq(itoBuildRecords.id, id)),
    });
  ...
  async updateBuildRecord(
    scope: StaffScope,
    id: string,
    input: UpdateItoBuildRecordRequest,
  ): Promise<ItoBuildRecordSummary> {
    await this.ensureBuildRecord(scope.companyId, id);
    const [updated] = await this.deps.db
      .update(itoBuildRecords)
      .set({ ...mapUpdateBuildRecordInput(input) })
      .where(and(eq(itoBuildRecords.companyId, scope.companyId), eq(itoBuildRecords.id, id)))
      .returning();
  ...
  async createTestRun(
    scope: StaffScope,
    input: CreateItoTestRunRequest,
  ): Promise<ItoTestRunSummary> {
    const [created] = await this.deps.db
      .insert(itoTestRuns)
      .values({ companyId: scope.companyId, ...mapCreateTestRunInput(input) })
      .returning();
  ...
  async listTestRuns(companyId: string): Promise<ItoTestRunSummary[]> {
    const rows = await this.deps.db.query.itoTestRuns.findMany({
      where: eq(itoTestRuns.companyId, companyId),
      orderBy: [desc(itoTestRuns.createdAt)],
      limit: 100,
    });
    return rows.map(toTestRunSummary);
  }
  async getTestRun(companyId: string, id: string): Promise<ItoTestRunSummary | null> {
    const row = await this.deps.db.query.itoTestRuns.findFirst({
      where: and(eq(itoTestRuns.companyId, companyId), eq(itoTestRuns.id, id)),
    });
  ...
  async updateTestRun(
    scope: StaffScope,
    id: string,
    input: UpdateItoTestRunRequest,
  ): Promise<ItoTestRunSummary> {
    await this.ensureTestRun(scope.companyId, id);
    const [updated] = await this.deps.db
      .update(itoTestRuns)
      .set({ ...mapUpdateTestRunInput(input) })
      .where(and(eq(itoTestRuns.companyId, scope.companyId), eq(itoTestRuns.id, id)))
      .returning();
  ...
  async createChangeRequest(
    scope: StaffScope,
    input: CreateItoChangeRequestRequest,
  ): Promise<ItoChangeRequestSummary> {
    const [created] = await this.deps.db
      .insert(itoChangeRequests)
      .values({ companyId: scope.companyId, ...mapCreateChangeRequestInput(input, scope) })
      .returning();
  ...
  async listChangeRequests(companyId: string): Promise<ItoChangeRequestSummary[]> {
    const rows = await this.deps.db.query.itoChangeRequests.findMany({
      where: eq(itoChangeRequests.companyId, companyId),
      orderBy: [desc(itoChangeRequests.createdAt)],
      limit: 100,
    });
  ...
  async getChangeRequest(companyId: string, id: string): Promise<ItoChangeRequestSummary | null> {
    const row = await this.deps.db.query.itoChangeRequests.findFirst({
      where: and(eq(itoChangeRequests.companyId, companyId), eq(itoChangeRequests.id, id)),
    });
  ...
  async updateChangeRequest(
    scope: StaffScope,
    id: string,
    input: UpdateItoChangeRequestRequest,
  ): Promise<ItoChangeRequestSummary> {
    await this.ensureChangeRequest(scope.companyId, id);
    const [updated] = await this.deps.db
      .update(itoChangeRequests)
      .set({ ...mapUpdateChangeRequestInput(input), updatedAt: new Date() })
      .where(and(eq(itoChangeRequests.companyId, scope.companyId), eq(itoChangeRequests.id, id)))
      .returning();
  ...
  async createDeployment(
    scope: StaffScope,
    input: CreateItoDeploymentRequest,
  ): Promise<ItoDeploymentSummary> {
    const [created] = await this.deps.db
      .insert(itoDeployments)
      .values({ companyId: scope.companyId, ...mapCreateDeploymentInput(input) })
      .returning();
  ...
  async listDeployments(companyId: string): Promise<ItoDeploymentSummary[]> {
    const rows = await this.deps.db.query.itoDeployments.findMany({
      where: eq(itoDeployments.companyId, companyId),
      orderBy: [desc(itoDeployments.createdAt)],
      limit: 100,
    });
    return rows.map(toDeploymentSummary);
  }
  async getDeployment(companyId: string, id: string): Promise<ItoDeploymentSummary | null> {
    const row = await this.deps.db.query.itoDeployments.findFirst({
      where: and(eq(itoDeployments.companyId, companyId), eq(itoDeployments.id, id)),
    });
  ...
  async updateDeployment(
    scope: StaffScope,
    id: string,
    input: UpdateItoDeploymentRequest,
  ): Promise<ItoDeploymentSummary> {
    await this.ensureDeployment(scope.companyId, id);
    const [updated] = await this.deps.db
      .update(itoDeployments)
      .set({ ...mapUpdateDeploymentInput(input), updatedAt: new Date() })
      .where(and(eq(itoDeployments.companyId, scope.companyId), eq(itoDeployments.id, id)))
      .returning();
  ...
  async createDependencyRecord(
    scope: StaffScope,
    input: CreateItoDependencyRecordRequest,
  ): Promise<ItoDependencyRecordSummary> {
    const [created] = await this.deps.db
      .insert(itoDependencyRecords)
      .values({ companyId: scope.companyId, ...mapCreateDependencyRecordInput(input) })
      .returning();
    await this.recordAudit(
      scope,
      'dependency_records_created',
      'ito_dependency_records',
      created!.id,
    );
    return toDependencyRecordSummary(created!);
  }
  async listDependencyRecords(companyId: string): Promise<ItoDependencyRecordSummary[]> {
    const rows = await this.deps.db.query.itoDependencyRecords.findMany({
      where: eq(itoDependencyRecords.companyId, companyId),
      orderBy: [desc(itoDependencyRecords.createdAt)],
      limit: 100,
    });
    return rows.map(toDependencyRecordSummary);
  }
  async getDependencyRecord(
    companyId: string,
    id: string,
  ): Promise<ItoDependencyRecordSummary | null> {
    const row = await this.deps.db.query.itoDependencyRecords.findFirst({
      where: and(eq(itoDependencyRecords.companyId, companyId), eq(itoDependencyRecords.id, id)),
    });
    return row ? toDependencyRecordSummary(row) : null;
  }
  async updateDependencyRecord(
    scope: StaffScope,
    id: string,
    input: UpdateItoDependencyRecordRequest,
  ): Promise<ItoDependencyRecordSummary> {
    await this.ensureDependencyRecord(scope.companyId, id);
    const [updated] = await this.deps.db
      .update(itoDependencyRecords)
      .set({ ...mapUpdateDependencyRecordInput(input), updatedAt: new Date() })
      .where(
        and(eq(itoDependencyRecords.companyId, scope.companyId), eq(itoDependencyRecords.id, id)),
      )
      .returning();
  ...
  async createTechnicalDebtRecord(
    scope: StaffScope,
    input: CreateItoTechnicalDebtRecordRequest,
  ): Promise<ItoTechnicalDebtRecordSummary> {
    const [created] = await this.deps.db
      .insert(itoTechnicalDebtRecords)
      .values({ companyId: scope.companyId, ...mapCreateTechnicalDebtRecordInput(input, scope) })
      .returning();
    await this.recordAudit(
      scope,
      'technical_debt_records_created',
      'ito_technical_debt_records',
      created!.id,
    );
  ...
  async listTechnicalDebtRecords(companyId: string): Promise<ItoTechnicalDebtRecordSummary[]> {
    const rows = await this.deps.db.query.itoTechnicalDebtRecords.findMany({
      where: eq(itoTechnicalDebtRecords.companyId, companyId),
      orderBy: [desc(itoTechnicalDebtRecords.createdAt)],
      limit: 100,
    });
    return rows.map(toTechnicalDebtRecordSummary);
  }
  async getTechnicalDebtRecord(
    companyId: string,
    id: string,
  ): Promise<ItoTechnicalDebtRecordSummary | null> {
    const row = await this.deps.db.query.itoTechnicalDebtRecords.findFirst({
      where: and(
        eq(itoTechnicalDebtRecords.companyId, companyId),
        eq(itoTechnicalDebtRecords.id, id),
      ),
    });
    return row ? toTechnicalDebtRecordSummary(row) : null;
  }
  async updateTechnicalDebtRecord(
    scope: StaffScope,
    id: string,
    input: UpdateItoTechnicalDebtRecordRequest,
  ): Promise<ItoTechnicalDebtRecordSummary> {
    await this.ensureTechnicalDebtRecord(scope.companyId, id);
    const [updated] = await this.deps.db
      .update(itoTechnicalDebtRecords)
      .set({ ...mapUpdateTechnicalDebtRecordInput(input), updatedAt: new Date() })
      .where(
        and(
          eq(itoTechnicalDebtRecords.companyId, scope.companyId),
          eq(itoTechnicalDebtRecords.id, id),
        ),
      )
      .returning();
    await this.recordAudit(
      scope,
      'technical_debt_records_updated',
      'ito_technical_debt_records',
      id,
    );
  ...
  async createBackupVerification(
    scope: StaffScope,
    input: CreateItoBackupVerificationRequest,
  ): Promise<ItoBackupVerificationSummary> {
    const [created] = await this.deps.db
      .insert(itoBackupVerifications)
      .values({ companyId: scope.companyId, ...mapCreateBackupVerificationInput(input, scope) })
      .returning();
    await this.recordAudit(
      scope,
      'backup_verifications_created',
      'ito_backup_verifications',
      created!.id,
    );
  ...
  async listBackupVerifications(companyId: string): Promise<ItoBackupVerificationSummary[]> {
    const rows = await this.deps.db.query.itoBackupVerifications.findMany({
      where: eq(itoBackupVerifications.companyId, companyId),
      orderBy: [desc(itoBackupVerifications.createdAt)],
      limit: 100,
    });
  ...
  async getBackupVerification(
    companyId: string,
    id: string,
  ): Promise<ItoBackupVerificationSummary | null> {
    const row = await this.deps.db.query.itoBackupVerifications.findFirst({
      where: and(
        eq(itoBackupVerifications.companyId, companyId),
        eq(itoBackupVerifications.id, id),
      ),
    });
    return row ? toBackupVerificationSummary(row) : null;
  }
  async updateBackupVerification(
    scope: StaffScope,
    id: string,
    input: UpdateItoBackupVerificationRequest,
  ): Promise<ItoBackupVerificationSummary> {
    await this.ensureBackupVerification(scope.companyId, id);
    const [updated] = await this.deps.db
      .update(itoBackupVerifications)
      .set({ ...mapUpdateBackupVerificationInput(input) })
      .where(
        and(
          eq(itoBackupVerifications.companyId, scope.companyId),
          eq(itoBackupVerifications.id, id),
        ),
      )
      .returning();
  ...
  async listDatabaseHealthSnapshots(
    companyId: string,
  ): Promise<ItoDatabaseHealthSnapshotSummary[]> {
    const rows = await this.deps.db.query.itoDatabaseHealthSnapshots.findMany({
      where: eq(itoDatabaseHealthSnapshots.companyId, companyId),
      orderBy: [desc(itoDatabaseHealthSnapshots.capturedAt)],
      limit: 100,
    });
    return rows.map(toDatabaseHealthSnapshotSummary);
  }

  async listApiReliabilitySnapshots(
    companyId: string,
  ): Promise<ItoApiReliabilitySnapshotSummary[]> {
    const rows = await this.deps.db.query.itoApiReliabilitySnapshots.findMany({
      where: eq(itoApiReliabilitySnapshots.companyId, companyId),
      orderBy: [desc(itoApiReliabilitySnapshots.capturedAt)],
      limit: 100,
    });
  ...
  async listAiProviderHealthSnapshots(companyId: string): Promise<ItoAiProviderHealthSummary[]> {
    const rows = await this.deps.db.query.itoAiProviderHealth.findMany({
      where: eq(itoAiProviderHealth.companyId, companyId),
      orderBy: [desc(itoAiProviderHealth.capturedAt)],
      limit: 100,
    });
  ...
  async listIntegrationHealthSnapshots(companyId: string): Promise<ItoIntegrationHealthSummary[]> {
    const rows = await this.deps.db.query.itoIntegrationHealth.findMany({
      where: eq(itoIntegrationHealth.companyId, companyId),
      orderBy: [desc(itoIntegrationHealth.capturedAt)],
      limit: 100,
    });
  ...
  async listPerformanceSnapshots(companyId: string): Promise<ItoPerformanceSnapshotSummary[]> {
    const rows = await this.deps.db.query.itoPerformanceSnapshots.findMany({
      where: eq(itoPerformanceSnapshots.companyId, companyId),
      orderBy: [desc(itoPerformanceSnapshots.capturedAt)],
      limit: 100,
    });
  ...
  async acknowledgeItAlert(scope: StaffScope, alertId: string): Promise<ItoItAlertSummary> {
    const row = await this.deps.db.query.itoItAlerts.findFirst({
      where: and(eq(itoItAlerts.companyId, scope.companyId), eq(itoItAlerts.id, alertId)),
    });
    ...
    const [updated] = await this.deps.db
      ...
      .where(eq(itoItAlerts.id, alertId))
      .returning();
  ...
  async listAuditLogs(companyId: string, limit = 100) {
    const rows = await this.deps.db.query.itoAuditLogs.findMany({
      where: eq(itoAuditLogs.companyId, companyId),
      orderBy: [desc(itoAuditLogs.createdAt)],
      limit,
    });
    return rows.map((row) => ({
      id: row.id,
...
function toPlatformConfigSummary(
  row: typeof itoPlatformConfig.$inferSelect,
): ItoPlatformConfigSummary {
  return {
    healthThresholds: row.healthThresholds,
    monitoringConfig: row.monitoringConfig,
    healingPolicies: row.healingPolicies,
    deploymentStandards: row.deploymentStandards,
    alertRouting: row.alertRouting,
    changeManagementPolicy: row.changeManagementPolicy,
...
function toAnalyticsSummary(row: typeof itoAnalyticsSnapshots.$inferSelect): ItoAnalyticsSummary {
  return {
    ...
    degradedMonitorCount: row.degradedMonitorCount,
    openBugCount: row.openBugCount,
    pendingChangeRequestCount: row.pendingChangeRequestCount,
    failedDeploymentCount: row.failedDeploymentCount,
    technicalDebtCount: row.technicalDebtCount,
...
function toHealthMonitorSummary(
  row: typeof itoHealthMonitors.$inferSelect,
): ItoHealthMonitorSummary {
  return {
    id: row.id,
...
function mapCreateHealthMonitorInput(input: CreateItoHealthMonitorRequest) {
  return {
    monitorKey: input.monitorKey.trim(),
...
function mapUpdateHealthMonitorInput(input: UpdateItoHealthMonitorRequest) {
  return {
    ...
    ...(input.healthStatus !== undefined
      ? { healthStatus: input.healthStatus as typeof itoHealthMonitors.$inferInsert.healthStatus }
      : {}),
...
function toHealthSnapshotSummary(
  row: typeof itoHealthSnapshots.$inferSelect,
): ItoHealthSnapshotSummary {
  return {
    id: row.id,
...
function mapCreateHealthSnapshotInput(input: CreateItoHealthSnapshotRequest) {
  return {
    ...
    snapshotKey: input.snapshotKey.trim(),
    healthStatus: (input.healthStatus ??
      'unknown') as typeof itoHealthSnapshots.$inferInsert.healthStatus,
    metrics: input.metrics ?? {},
    config: input.config ?? {},
  };
}

function toSelfHealingActionSummary(
  row: typeof itoSelfHealingActions.$inferSelect,
): ItoSelfHealingActionSummary {
  return {
    id: row.id,
    monitorId: row.monitorId,
...
function mapCreateSelfHealingActionInput(input: CreateItoSelfHealingActionRequest) {
  return {
    ...
    riskLevel: (input.riskLevel ?? 'medium') as typeof itoSelfHealingActions.$inferInsert.riskLevel,
...
function mapUpdateSelfHealingActionInput(input: UpdateItoSelfHealingActionRequest) {
  return {
    ...
    ...(input.riskLevel !== undefined
      ? { riskLevel: input.riskLevel as typeof itoSelfHealingActions.$inferInsert.riskLevel }
      : {}),
    ...
    ...(input.workflowStatus !== undefined
      ? {
          workflowStatus:
            input.workflowStatus as typeof itoSelfHealingActions.$inferInsert.workflowStatus,
        }
      : {}),
...
function mapCreateBugDetectionInput(input: CreateItoBugDetectionRequest) {
  return {
    detectionSource: input.detectionSource.trim(),
    severity: (input.severity ?? 'warning') as typeof itoBugDetections.$inferInsert.severity,
    title: input.title.trim(),
...
function mapUpdateBugDetectionInput(input: UpdateItoBugDetectionRequest) {
  return {
    ...
    ...(input.severity !== undefined
      ? { severity: input.severity as typeof itoBugDetections.$inferInsert.severity }
      : {}),
    ...(input.title !== undefined ? { title: input.title.trim() } : {}),
    ...
    ...(input.workflowStatus !== undefined
      ? {
          workflowStatus:
            input.workflowStatus as typeof itoBugDetections.$inferInsert.workflowStatus,
        }
      : {}),
  };
}

function toRootCauseAnalysisSummary(
  row: typeof itoRootCauseAnalyses.$inferSelect,
): ItoRootCauseAnalysisSummary {
  return {
    id: row.id,
    bugDetectionId: row.bugDetectionId,
    incidentId: row.incidentId,
    title: row.title,
    rootCause: row.rootCause,
    workflowStatus: row.workflowStatus,
    analyzedByUserId: row.analyzedByUserId,
    completedAt: row.completedAt?.toISOString() ?? null,
  };
}
function mapCreateRootCauseAnalysisInput(
  input: CreateItoRootCauseAnalysisRequest,
  scope: StaffScope,
) {
  return {
    bugDetectionId: input.bugDetectionId ?? null,
    incidentId: input.incidentId ?? null,
    title: input.title.trim(),
    rootCause: input.rootCause ?? null,
    analysis: input.analysis ?? {},
    config: input.config ?? {},
    analyzedByUserId: scope.userId,
    workflowStatus: 'draft' as const,
  };
}
function mapUpdateRootCauseAnalysisInput(input: UpdateItoRootCauseAnalysisRequest) {
  return {
    ...
    ...(input.workflowStatus !== undefined
      ? {
          workflowStatus:
            input.workflowStatus as typeof itoRootCauseAnalyses.$inferInsert.workflowStatus,
        }
      : {}),
...
function toRepairAttemptSummary(
  row: typeof itoRepairAttempts.$inferSelect,
): ItoRepairAttemptSummary {
  return {
    id: row.id,
    bugDetectionId: row.bugDetectionId,
    rootCauseAnalysisId: row.rootCauseAnalysisId,
...
function mapCreateRepairAttemptInput(input: CreateItoRepairAttemptRequest, scope: StaffScope) {
  return {
    bugDetectionId: input.bugDetectionId ?? null,
    rootCauseAnalysisId: input.rootCauseAnalysisId ?? null,
    repairType: input.repairType.trim(),
    riskLevel: (input.riskLevel ?? 'medium') as typeof itoRepairAttempts.$inferInsert.riskLevel,
    notes: input.notes ?? null,
    config: input.config ?? {},
    attemptedByUserId: scope.userId,
    workflowStatus: 'draft' as const,
  };
}
function mapUpdateRepairAttemptInput(input: UpdateItoRepairAttemptRequest) {
  return {
    ...(input.bugDetectionId !== undefined ? { bugDetectionId: input.bugDetectionId ?? null } : {}),
    ...(input.rootCauseAnalysisId !== undefined
      ? { rootCauseAnalysisId: input.rootCauseAnalysisId ?? null }
      : {}),
    ...(input.repairType !== undefined ? { repairType: input.repairType } : {}),
    ...(input.riskLevel !== undefined
      ? { riskLevel: input.riskLevel as typeof itoRepairAttempts.$inferInsert.riskLevel }
      : {}),
    ...
    ...(input.workflowStatus !== undefined
      ? {
          workflowStatus:
            input.workflowStatus as typeof itoRepairAttempts.$inferInsert.workflowStatus,
        }
      : {}),
    ...(input.success !== undefined ? { success: input.success } : {}),
    ...(input.attemptedAt !== undefined
      ? { attemptedAt: parseOptionalDate(input.attemptedAt) }
      : {}),
  };
}
...
function mapUpdateBuildRecordInput(input: UpdateItoBuildRecordRequest) {
  return {
    ...
    ...(input.workflowStatus !== undefined
      ? {
          workflowStatus:
            input.workflowStatus as typeof itoBuildRecords.$inferInsert.workflowStatus,
        }
      : {}),
...
function mapUpdateTestRunInput(input: UpdateItoTestRunRequest) {
  return {
    ...
    ...(input.config !== undefined ? { config: input.config } : {}),
    ...(input.workflowStatus !== undefined
      ? { workflowStatus: input.workflowStatus as typeof itoTestRuns.$inferInsert.workflowStatus }
      : {}),
...
function toChangeRequestSummary(
  row: typeof itoChangeRequests.$inferSelect,
): ItoChangeRequestSummary {
  return {
    id: row.id,
...
function mapCreateChangeRequestInput(input: CreateItoChangeRequestRequest, scope: StaffScope) {
  return {
    changeNumber: input.changeNumber ?? null,
    title: input.title.trim(),
    description: input.description ?? null,
    riskLevel: (input.riskLevel ?? 'medium') as typeof itoChangeRequests.$inferInsert.riskLevel,
...
function mapUpdateChangeRequestInput(input: UpdateItoChangeRequestRequest) {
  return {
    ...
    ...(input.description !== undefined ? { description: input.description ?? null } : {}),
    ...(input.riskLevel !== undefined
      ? { riskLevel: input.riskLevel as typeof itoChangeRequests.$inferInsert.riskLevel }
      : {}),
    ...
    ...(input.workflowStatus !== undefined
      ? {
          workflowStatus:
            input.workflowStatus as typeof itoChangeRequests.$inferInsert.workflowStatus,
        }
      : {}),
...
function mapUpdateDeploymentInput(input: UpdateItoDeploymentRequest) {
  return {
    ...
    ...(input.changeRequestId !== undefined
      ...
      : {}),
    ...(input.config !== undefined ? { config: input.config } : {}),
    ...(input.deploymentStatus !== undefined
      ? {
          deploymentStatus:
            input.deploymentStatus as typeof itoDeployments.$inferInsert.deploymentStatus,
        }
      : {}),
    ...(input.startedAt !== undefined ? { startedAt: parseOptionalDate(input.startedAt) } : {}),
    ...(input.completedAt !== undefined
...
function toDependencyRecordSummary(
  row: typeof itoDependencyRecords.$inferSelect,
): ItoDependencyRecordSummary {
  return {
    id: row.id,
...
function mapCreateDependencyRecordInput(input: CreateItoDependencyRecordRequest) {
  return {
    dependencyName: input.dependencyName.trim(),
    dependencyType: input.dependencyType.trim(),
    version: input.version ?? null,
...
function mapUpdateDependencyRecordInput(input: UpdateItoDependencyRecordRequest) {
  return {
    ...
    ...(input.healthStatus !== undefined
      ? {
          healthStatus: input.healthStatus as typeof itoDependencyRecords.$inferInsert.healthStatus,
        }
      : {}),
  };
}

function toTechnicalDebtRecordSummary(
  row: typeof itoTechnicalDebtRecords.$inferSelect,
): ItoTechnicalDebtRecordSummary {
  return {
    id: row.id,
    debtKey: row.debtKey,
    ...
    estimatedEffortHours:
      row.estimatedEffortHours != null ? String(row.estimatedEffortHours) : null,
    ownerUserId: row.ownerUserId,
  };
}
function mapCreateTechnicalDebtRecordInput(
  input: CreateItoTechnicalDebtRecordRequest,
  scope: StaffScope,
) {
  return {
    debtKey: input.debtKey.trim(),
    title: input.title.trim(),
    category: input.category.trim(),
    severity: (input.severity ?? 'medium') as typeof itoTechnicalDebtRecords.$inferInsert.severity,
    estimatedEffortHours:
      input.estimatedEffortHours != null ? String(input.estimatedEffortHours) : null,
    description: input.description ?? null,
    config: input.config ?? {},
...
function mapUpdateTechnicalDebtRecordInput(input: UpdateItoTechnicalDebtRecordRequest) {
  return {
    ...
    ...(input.severity !== undefined
      ? { severity: input.severity as typeof itoTechnicalDebtRecords.$inferInsert.severity }
      : {}),
    ...
    ...(input.workflowStatus !== undefined
      ? {
          workflowStatus:
            input.workflowStatus as typeof itoTechnicalDebtRecords.$inferInsert.workflowStatus,
        }
      : {}),
  };
}

function toBackupVerificationSummary(
  row: typeof itoBackupVerifications.$inferSelect,
): ItoBackupVerificationSummary {
  return {
    id: row.id,
...
function mapCreateBackupVerificationInput(
  input: CreateItoBackupVerificationRequest,
  scope: StaffScope,
) {
  return {
    backupRef: input.backupRef.trim(),
    notes: input.notes ?? null,
    config: input.config ?? {},
...
function mapUpdateBackupVerificationInput(input: UpdateItoBackupVerificationRequest) {
  return {
    ...
    ...(input.verificationStatus !== undefined
      ? {
          verificationStatus:
            input.verificationStatus as typeof itoBackupVerifications.$inferInsert.verificationStatus,
        }
      : {}),
...
function toDatabaseHealthSnapshotSummary(
  row: typeof itoDatabaseHealthSnapshots.$inferSelect,
): ItoDatabaseHealthSnapshotSummary {
  return {
    id: row.id,
    healthStatus: row.healthStatus,
    connectionPoolUsagePercent:
      row.connectionPoolUsagePercent != null ? String(row.connectionPoolUsagePercent) : null,
...
function toApiReliabilitySnapshotSummary(
  row: typeof itoApiReliabilitySnapshots.$inferSelect,
): ItoApiReliabilitySnapshotSummary {
  return {
    id: row.id,
    endpointGroup: row.endpointGroup,
    healthStatus: row.healthStatus,
    availabilityPercent: row.availabilityPercent != null ? String(row.availabilityPercent) : null,
    errorRatePercent: row.errorRatePercent != null ? String(row.errorRatePercent) : null,
    p95LatencyMs: row.p95LatencyMs,
    capturedAt: row.capturedAt.toISOString(),
  };
}
function toAiProviderHealthSummary(
  row: typeof itoAiProviderHealth.$inferSelect,
): ItoAiProviderHealthSummary {
  return {
    id: row.id,
...
function toIntegrationHealthSummary(
  row: typeof itoIntegrationHealth.$inferSelect,
): ItoIntegrationHealthSummary {
  return {
    id: row.id,
...
function toPerformanceSnapshotSummary(
  row: typeof itoPerformanceSnapshots.$inferSelect,
): ItoPerformanceSnapshotSummary {
  return {
    id: row.id,
    healthStatus: row.healthStatus,
    cpuUsagePercent: row.cpuUsagePercent != null ? String(row.cpuUsagePercent) : null,
