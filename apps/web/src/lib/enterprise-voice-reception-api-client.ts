...
export async function fetchVoiceReceptionDashboard(
  accessToken: string,
): Promise<EnterpriseVoiceReceptionDashboard> {
  const data = await request<{ dashboard: EnterpriseVoiceReceptionDashboard }>(
    '/enterprise-voice-reception/dashboard',
    {
      accessToken,
    },
  );
...
export async function syncVoiceAlerts(accessToken: string): Promise<VrVoiceAlertSummary[]> {
  const data = await request<{ voiceAlerts: VrVoiceAlertSummary[] }>(
    '/enterprise-voice-reception/voice-alerts/sync',
    {
      accessToken,
      method: 'POST',
    },
...
export async function captureVoiceAnalytics(accessToken: string) {
  const data = await request<{ analytics: unknown }>(
    '/enterprise-voice-reception/analytics/capture',
    {
      accessToken,
      method: 'POST',
    },
...
export async function fetchVoiceAuditLogs(accessToken: string): Promise<VrAuditLogSummary[]> {
  const data = await request<{ auditLogs: VrAuditLogSummary[] }>(
    '/enterprise-voice-reception/audit-logs',
    {
      accessToken,
    },
  );
...
export async function fetchVoiceActionDrafts(accessToken: string): Promise<VrActionDraftSummary[]> {
  const data = await request<{ actionDrafts: VrActionDraftSummary[] }>(
    '/enterprise-voice-reception/action-drafts',
    {
      accessToken,
    },
  );
...
export async function updateAiReceptionistConfig(
  ...
  const data = await request<{ aiReceptionist: unknown }>(
    '/enterprise-voice-reception/ai-receptionist',
    {
      accessToken,
      method: 'PUT',
      body: input,
