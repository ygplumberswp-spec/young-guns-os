...
export async function fetchQualityDashboard(accessToken: string) {
  const data = await request<{ dashboard: QualityExecutiveDashboard }>('/quality/dashboard', { accessToken });
  return data.dashboard;
}

export async function fetchQualityComebacks(accessToken: string) {
  const data = await request<{ comebacks: QualityComebackSummary[] }>('/quality/comebacks', { accessToken });
  return data.comebacks;
}

export async function fetchQualityComeback(accessToken: string, id: string) {
  const data = await request<{ comeback: QualityComebackSummary }>(`/quality/comebacks/${id}`, { accessToken });
  return data.comeback;
}

export async function createQualityComeback(accessToken: string, body: CreateQualityComebackRequest) {
...
export async function fetchQualityWarrantyClaims(accessToken: string) {
  const data = await request<{ claims: QualityWarrantyClaimSummary[] }>('/quality/warranty', { accessToken });
  return data.claims;
}

export async function createQualityWarrantyClaim(accessToken: string, body: CreateQualityWarrantyClaimRequest) {
...
export async function fetchQualityTechnicianIntelligence(accessToken: string) {
  const data = await request<{ intelligence: QualityTechnicianIntelligence }>('/quality/technicians', {
    accessToken,
  });
...
export async function createQualitySupplierDefect(accessToken: string, body: CreateQualitySupplierDefectRequest) {
...
export async function fetchQualityActions(accessToken: string) {
  const data = await request<{ actions: QualityActionSummary[] }>('/quality/actions', { accessToken });
  return data.actions;
}
