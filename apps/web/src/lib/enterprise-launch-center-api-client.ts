...
export async function fetchLaunchCenterDashboard(
  accessToken: string,
): Promise<EnterpriseLaunchCenterDashboard> {
  const data = await request<{ dashboard: EnterpriseLaunchCenterDashboard }>(
    '/enterprise-launch-center/dashboard',
    {
      accessToken,
    },
  );
...
export async function approveGoLiveWizard(accessToken: string, wizardId: string, notes?: string) {
  const data = await request<{ wizard: unknown }>(
    `/enterprise-launch-center/go-live/wizards/${wizardId}/approve`,
    {
      accessToken,
      method: 'POST',
      body: { notes },
...
export async function runPostDeploymentValidation(accessToken: string, goLiveWizardId?: string) {
  const data = await request<{ validation: unknown }>(
    '/enterprise-launch-center/deployment-validations/run',
    {
      accessToken,
      method: 'POST',
...
export async function syncLaunchCenterAlerts(accessToken: string) {
  const data = await request<{ platformAlerts: unknown[] }>(
    '/enterprise-launch-center/platform-alerts/sync',
    {
      accessToken,
      method: 'POST',
    },
...
export async function captureLaunchCenterAnalytics(accessToken: string) {
  const data = await request<{ analytics: unknown }>(
    '/enterprise-launch-center/analytics/capture',
    {
      accessToken,
      method: 'POST',
    },
...
export async function fetchLaunchCenterAuditLogs(
  accessToken: string,
): Promise<LncAuditLogSummary[]> {
  const data = await request<{ auditLogs: LncAuditLogSummary[] }>(
    '/enterprise-launch-center/audit-logs',
    {
      accessToken,
    },
  );
...
export async function validateRollbackPlan(accessToken: string, linkId: string) {
  const data = await request<{ link: unknown }>(
    `/enterprise-launch-center/rollback-plans/${linkId}/validate`,
    {
      accessToken,
      method: 'POST',
    },
