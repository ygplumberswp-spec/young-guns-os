...
export async function fetchKnowledgeGraphDashboard(accessToken: string) {
  const data = await request<{ dashboard: EnterpriseKnowledgeGraphDashboard }>('/knowledge-graph/dashboard', {
    accessToken,
  });
...
export async function syncKnowledgeGraph(accessToken: string) {
  const data = await request<{ entityCount: number; relationshipCount: number }>('/knowledge-graph/sync', {
    accessToken,
    method: 'POST',
  });
  return data;
...
export async function searchKnowledgeGraph(accessToken: string, query: string) {
  const data = await request<{ results: KnowledgeSemanticSearchResult[] }>('/knowledge-graph/search', {
    accessToken,
    method: 'POST',
...
export async function generateKnowledgeGraphRecommendations(accessToken: string) {
  const data = await request<{ recommendations: EnterpriseKnowledgeGraphDashboard['recommendations'] }>(
    '/knowledge-graph/recommendations/generate',
