...
export async function captureCommunicationsAnalytics(accessToken: string) {
  const data = await request<{ snapshot: unknown }>(
    '/enterprise-communications/analytics/capture',
    {
      method: 'POST',
      accessToken,
    },
...
export async function createOutboundCallCampaign(
  ...
  const data = await request<{ campaign: unknown }>(
    '/enterprise-communications/outbound-campaigns',
    {
      method: 'POST',
      accessToken,
      body,
...
export async function updateCommunicationsPlatformConfig(
  accessToken: string,
  body: UpdateUcPlatformConfigRequest,
) {
  const data = await request<{ config: UcPlatformConfigSummary }>(
    '/enterprise-communications/config',
    {
      method: 'PATCH',
      accessToken,
      body,
...
export async function fetchCustomerCommunicationCenter(accessToken: string, customerId: string) {
  const data = await request<{ center: unknown }>(
    `/enterprise-communications/customers/${customerId}/center`,
    {
      accessToken,
    },
  );
